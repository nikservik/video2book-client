...
Для этого проекта не используем сертификаты и notarization:

- macOS: собираем `dmg` без `Developer ID` и без notarization; `electron-builder` может применить только ad-hoc signing bundle;
- Windows: собираем `nsis` без code signing certificate.

Причина: приложение распространяется только внутри небольшой команды. Поэтому мы принимаем platform security warnings и не усложняем процесс сертификатами и notarization.
